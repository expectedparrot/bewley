...
from bewley.commands.common import (
    HumanOption,
    action,
    fail,
    finish,
...
app = typer.Typer(
    help="Build auditable qualitative insight workflows and deterministic reports."
)
discover_app = typer.Typer(
    help="Discover candidate codes from reproducible bundles of responses."
)
consolidate_app = typer.Typer(
    help="Consolidate discovered candidates into a compact frozen codebook."
)
classify_app = typer.Typer(
    help="Classify every response against a frozen feedback codebook."
)
...
def _edsl():
    try:
        ...
        raise BewleyError(
            "EDSL is required for rapid insights", code="DEPENDENCY_MISSING"
        ) from exc
...
def _configured_model(Model, name: str, max_tokens: int):
    ...
    return Model(
        name, service_name=service, max_tokens=max_tokens, reasoning_effort="low"
    )


def _save_model_list_json(
    Model, ModelList, name: str, max_tokens: int, target: Path, force: bool
) -> Path:
...
def _feedback_text(text: str) -> str:
    ...
    marker = next(
        (
            ...
            if "[Feedback about the AI interviewer]" in line
        ),
        None,
    )
    if marker is None:
...
def _feedback_documents(project) -> list[dict[str, str]]:
    with project.connect() as conn:
        ...
        for row in rows:
            ...
            text = safe_decode(
                (project.objects_dir / revision["content_sha256"]).read_bytes()
            )
...
def _themes(project) -> list[dict[str, Any]]:
    ...
    if not rows:
        raise BewleyError(
            "Rapid insights requires an applied focused codebook.",
            code="MISSING_FOCUSED_CODEBOOK",
...
def _bundle_rows(
    documents: list[dict[str, str]], seed: int, bundle_size: int, coverage: int
...
def discovery_jobs_command(
    ...
    try:
        ...
        for index, bundle in enumerate(bundles, 1):
            ...
            manifest_bundles.append(
                {"bundle_id": bundle_id, "source_ids": list(source_map)}
            )
        question = QuestionFreeText(
            question_name=DISCOVERY_QUESTION_NAME, question_text=DISCOVERY_QUESTION_TEXT
        )
        jobs = Jobs(survey=question.to_survey()).by(ScenarioList(scenarios))
        model_target = None
        if model:
            model_target = _save_model_list_json(
                Model,
                ModelList,
                model,
                max_tokens,
                target.with_name("models.json"),
                force,
            )
        target.parent.mkdir(parents=True, exist_ok=True)
        if target.exists():
        ...
        data = {
            ...
            "coverage": coverage,
            "expected_model_calls": len(bundles),
            "models": str(model_target) if model_target else None,
            "saved": saved,
        }
    ...
    finish(
        ...
        next_actions=[
            action(
                "run-feedback-discovery",
                "Run bundled code discovery externally",
                [
                    "ep",
                    "run",
                    str(target),
                    *model_args,
                    "--output",
                    str(target.with_name("results.ep")),
                ],
                mutates_state=True,
...
def _parse_discovery(
    raw: Any, sources: dict[str, dict[str, str]]
) -> list[dict[str, Any]]:
    ...
    for item in raw:
        ...
        for row in evidence:
            ...
            if (
                ...
                or not exact.strip()
                or exact not in sources[row["source_id"]]["feedback_text"]
            ):
...
def discovery_ingest_command(
    ...
    output: Path = typer.Option(
        Path("runs/001-discovery/candidates.jsonl"), "--output", "-o"
    ),
    force: bool = typer.Option(False, "--force"),
    human: bool = HumanOption,
) -> None:
    ...
    project = get_project(command, json_flag)
    result_path, jobs_path, target = (
        _path(project.root, value) for value in (results, jobs, output)
    )
    try:
        if target.exists() and not force:
            raise BewleyError(f"{target} already exists", code="ALREADY_EXISTS")
        Jobs, _, _, _, Results, _, _ = _edsl()
        source_scenarios = list(Jobs.git.load(jobs_path).scenarios)
        expected = {dict(s)["bundle_id"] for s in source_scenarios}
        rows, seen, failures, rejected = [], set(), [], []
        for result in validate_result_scenarios(
            Results.git.load(result_path), source_scenarios
        ):
            ...
            try:
                ...
                raw = (
                    answer.get(DISCOVERY_QUESTION_NAME)
                        ...
                        rejected.append(
                            {
                                ...
                                "code_key": candidate.get("code_key")
        ...
        if failures or seen != expected:
            raise BewleyError(
                "Discovery results failed validation; no output was written.",
                code="INCOMPLETE_RESULTS",
                context={
                    "failures": failures,
                    "missing_bundles": sorted(expected - seen),
                },
            )
        target.parent.mkdir(parents=True, exist_ok=True)
...
def consolidation_jobs_command(
    candidates: Path = typer.Option(
        Path("runs/001-discovery-retry2/candidates.jsonl"), "--candidates"
    ),
    output: Path = typer.Option(
        Path("runs/002-consolidation/jobs.ep"), "--output", "-o"
    ),
    ...
    try:
        if min_codes > max_codes:
            raise BewleyError(
                "--min-codes cannot exceed --max-codes", code="INVALID_INPUT"
            )
        if target.exists() and not force:
            raise BewleyError(f"{target} already exists", code="ALREADY_EXISTS")
        ...
        if not inventory:
            raise BewleyError(
                "No validated discovery candidates found.", code="INVALID_INPUT"
            )
        Jobs, Model, ModelList, QuestionFreeText, Results, Scenario, ScenarioList = (
            _edsl()
        )
        correction_context = ""
        if prior_results:
            ...
            if len(prior_rows) != 1:
                raise BewleyError(
                    "--prior-results must contain one result", code="INVALID_INPUT"
                )
            answer = prior_rows[0]["answer"]
            prior_raw = (
                answer.get(CONSOLIDATION_QUESTION_NAME)
                if isinstance(answer, dict)
                else ""
            )
            prior_value = json.loads(
                prior_raw[prior_raw.find("{") : prior_raw.rfind("}") + 1]
            )
            expected_ids = {row["candidate_id"] for row in inventory}
            mapped_ids = [
                item
                for code in prior_value.get("codes", [])
                for item in code.get("candidate_ids", [])
            ]
        ...
        question = QuestionFreeText(
            question_name=CONSOLIDATION_QUESTION_NAME,
            question_text=CONSOLIDATION_QUESTION_TEXT,
        )
        ...
        error = (
            exc
            if isinstance(exc, BewleyError)
            else BewleyError(str(exc), code="INVALID_INPUT")
        )
    ...
    finish(
        ...
        next_actions=[
            action(
                "run-feedback-consolidation",
                "Run global candidate consolidation externally",
                [
                    "ep",
                    "run",
                    "--jobs",
                    str(target),
                    "--model_list",
                    str(model_target or target.with_name("models.json")),
                    "--output",
                    str(target.with_name("results.ep")),
                ],
                mutates_state=True,
...
def _parse_consolidation(
    raw: Any, expected: set[str], minimum: int, maximum: int
) -> dict[str, Any]:
    if isinstance(raw, str):
        start, end = raw.find("{"), raw.rfind("}")
        if start < 0 or end < start:
    ...
    for theme in themes:
        if not isinstance(theme, dict) or not re.fullmatch(
            r"[a-z][a-z0-9_]{2,63}", str(theme.get("theme_key", ""))
        ):
            raise ValueError("invalid theme")
        if not all(
            isinstance(theme.get(key), str) and theme[key].strip()
    ...
    for code in codes:
        ...
        if not isinstance(code, dict) or not all(
            isinstance(code.get(key), str) and code[key].strip() for key in fields
        ):
            raise ValueError("incomplete code")
        if (
            not re.fullmatch(r"[a-z][a-z0-9_]{2,63}", code["code_key"])
            or code["theme_key"] not in theme_keys
        ):
...
def consolidation_ingest_command(
    ...
    output: Path = typer.Option(
        Path("qualitative-analysis/feedback-codebook.json"), "--output", "-o"
    ),
    force: bool = typer.Option(False, "--force"),
    human: bool = HumanOption,
) -> None:
    ...
    try:
        if target.exists() and not force:
            raise BewleyError(f"{target} already exists", code="ALREADY_EXISTS")
        Jobs, _, _, _, Results, _, _ = _edsl()
        scenarios = list(Jobs.git.load(jobs_path).scenarios)
        result_rows = validate_result_scenarios(
            Results.git.load(result_path), scenarios
        )
        if len(scenarios) != 1 or len(result_rows) != 1:
            raise BewleyError(
                "Expected exactly one consolidation result.", code="INCOMPLETE_RESULTS"
            )
        scenario = dict(scenarios[0])
        answer = result_rows[0]["answer"]
        raw = (
            answer.get(CONSOLIDATION_QUESTION_NAME)
            if isinstance(answer, dict)
            else None
        )
        codebook = _parse_consolidation(
            raw,
            set(scenario["candidate_ids"]),
            scenario["min_codes"],
            scenario["max_codes"],
        )
        artifact = {
        ...
        artifact["codebook_fingerprint"] = hashlib.sha256(
            json.dumps(codebook, sort_keys=True).encode()
        ).hexdigest()
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_text(
        ...
        error = (
            exc
            if isinstance(exc, BewleyError)
            else BewleyError(str(exc), code="INVALID_RESULTS")
        )
        fail(command, error, json_flag)
        return
    finish(command, data)
...
def classification_jobs_command(
    codebook: Path = typer.Option(
        Path("qualitative-analysis/feedback-codebook.json"), "--codebook"
    ),
    output: Path = typer.Option(
        Path("runs/003-classification/jobs.ep"), "--output", "-o"
    ),
    model: Optional[str] = typer.Option(None, "--model"),
    ...
    try:
        ...
        scenarios = [
            Scenario(
                {
                    **row,
                    "response_text": row["feedback_text"],
                    "codebook_fingerprint": artifact["codebook_fingerprint"],
        ...
        question = QuestionFreeText(
            question_name=CLASSIFICATION_QUESTION_NAME,
        ...
        data = {
            "output": str(target),
            "document_count": len(documents),
            "code_count": len(artifact["codes"]),
            "codebook_fingerprint": artifact["codebook_fingerprint"],
            "expected_model_calls": len(documents),
            "models": str(model_target) if model_target else None,
            "saved": saved,
        }
        record_command_artifacts(project, command, locals())
    ...
    finish(
        ...
        next_actions=[
            action(
                "run-feedback-classification",
                "Classify every feedback response externally",
                [
                    "ep",
                    "run",
                    "--jobs",
...
def _parse_classification(raw: Any, text: str, allowed: set[str]) -> dict[str, Any]:
    ...
    for item in assignments:
        ...
        if item.get("code_key") not in allowed:
            rejected.append({"assignment": item, "reason": "unknown code_key"})
            continue
        if item["code_key"] in seen:
            rejected.append({"assignment": item, "reason": "duplicate code_key"})
            continue
    ...
    return {
        ...
        "potential_new_theme": novel,
        "_rejected_assignments": rejected,
    }
...
def classification_ingest_command(
    ...
    output: Path = typer.Option(
        Path("qualitative-analysis/feedback-classifications.jsonl"), "--output", "-o"
    ),
    force: bool = typer.Option(False, "--force"),
    human: bool = HumanOption,
) -> None:
    ...
    project = get_project(command, json_flag)
    result_path, jobs_path, codebook_path, target = (
        _path(project.root, value) for value in (results, jobs, codebook, output)
    )
    try:
        if target.exists() and not force:
            raise BewleyError(f"{target} already exists", code="ALREADY_EXISTS")
        artifact = json.loads(codebook_path.read_text(encoding="utf-8"))
        allowed = {row["code_key"] for row in artifact["codes"]}
        Jobs, _, _, _, Results, _, _ = _edsl()
        source_scenarios = list(Jobs.git.load(jobs_path).scenarios)
        expected = {dict(s)["document_id"] for s in source_scenarios}
        rows, seen, failures, rejected_assignments = [], set(), [], []
        for result in validate_result_scenarios(
            Results.git.load(result_path), source_scenarios
        ):
            scenario = dict(result["scenario"])
            document_id = scenario.get("document_id", "unknown")
            try:
                if document_id in seen:
                    raise ValueError("duplicate document result")
                if scenario["codebook_fingerprint"] != artifact["codebook_fingerprint"]:
                    raise ValueError("stale codebook fingerprint")
                answer = result["answer"]
                raw = (
                    answer.get(CLASSIFICATION_QUESTION_NAME)
                    if isinstance(answer, dict)
                    else None
                )
                ...
                for rejected in parsed.pop("_rejected_assignments"):
                    rejected_assignments.append(
                        {"document_id": document_id, **rejected}
                    )
                rows.append(
                    {
                        "document_id": document_id,
                        "document_path": scenario["document_path"],
        ...
        if failures or seen != expected:
            raise BewleyError(
                "Classification results failed validation; no output was written.",
                code="INCOMPLETE_RESULTS",
                context={
                    "failures": failures,
                    "missing_document_count": len(expected - seen),
                },
            )
        target.parent.mkdir(parents=True, exist_ok=True)
        with target.open("w", encoding="utf-8") as handle:
            for row in sorted(rows, key=lambda value: value["document_path"]):
                handle.write(json.dumps(row, ensure_ascii=False) + "\n")
        ...
        data = {
            "output": str(target),
            "response_count": len(rows),
            "assignment_count": sum(len(row["assignments"]) for row in rows),
            "rejected_assignment_count": len(rejected_assignments),
            "rejected_assignments": rejected_assignments,
            "potential_new_theme_count": sum(
                row["potential_new_theme"] is not None for row in rows
            ),
            "ingest_log": str(log),
        }
        record_command_artifacts(project, command, locals())
...
def aggregate_command(
    classifications: Path = typer.Option(
        Path("qualitative-analysis/feedback-classifications.jsonl"), "--classifications"
    ),
    codebook: Path = typer.Option(
        Path("qualitative-analysis/feedback-codebook.json"), "--codebook"
    ),
    output: Path = typer.Option(
        Path("qualitative-analysis/feedback-aggregate.json"), "--output", "-o"
    ),
    human: bool = HumanOption,
) -> None:
    """Compute deterministic theme and sentiment tables from validated classifications."""
    command, json_flag = "insights aggregate", should_emit_json(human)
    project = get_project(command, json_flag)
    try:
        rows = [
            json.loads(line)
            for line in _path(project.root, classifications)
            .read_text(encoding="utf-8")
            .splitlines()
            if line.strip()
        ]
        book = json.loads(_path(project.root, codebook).read_text(encoding="utf-8"))
        codes = {row["code_key"]: row for row in book["codes"]}
        themes = {row["theme_key"]: row for row in book["themes"]}
        code_counts = Counter(
            item["code_key"] for row in rows for item in row["assignments"]
        )
        sentiment = Counter(row["sentiment"] for row in rows)
        theme_docs = {key: set() for key in themes}
        for row in rows:
            for key in {
                codes[item["code_key"]]["theme_key"] for item in row["assignments"]
            }:
                theme_docs[key].add(row["document_id"])
        artifact = {
            "schema_version": "1.0",
            "generated_at": utcnow(),
            "response_count": len(rows),
            "codebook_fingerprint": book["codebook_fingerprint"],
            ...
            "themes": [
                ...
                for key, ids in theme_docs.items()
            ],
            "codes": [
                {
                    "code_key": key,
                    "name": codes[key]["name"],
                    "theme_key": codes[key]["theme_key"],
                    "response_count": code_counts[key],
                }
                for key in codes
            ],
            "unassigned_response_count": sum(not row["assignments"] for row in rows),
            "potential_new_theme_count": sum(
                row["potential_new_theme"] is not None for row in rows
            ),
        }
        ...
        data = {
            ...
            **{
                ...
                for key in (
                    "response_count",
                    "unassigned_response_count",
                    "potential_new_theme_count",
                )
            },
        }
        record_command_artifacts(project, command, locals())
    except (OSError, KeyError, json.JSONDecodeError) as exc:
        fail(command, BewleyError(str(exc), code="INVALID_INPUT"), json_flag)
        return
    finish(command, data)
...
def evidence_export_command(
    aggregate: Path = typer.Option(
        Path("qualitative-analysis/feedback-aggregate.json"), "--aggregate"
    ),
    classifications: Path = typer.Option(
        Path("qualitative-analysis/feedback-classifications.jsonl"), "--classifications"
    ),
    ...
    output: Path = typer.Option(
        Path("qualitative-analysis/feedback-insights-explorer.html"), "--output", "-o"
    ),
    ...
    command, json_flag = "insights evidence-export", should_emit_json(human)
    project = get_project(command, json_flag)
    try:
        agg = json.loads(_path(project.root, aggregate).read_text(encoding="utf-8"))
        book = json.loads(_path(project.root, codebook).read_text(encoding="utf-8"))
        rows = [
            json.loads(line)
            for line in _path(project.root, classifications)
        ...
        for values in evidence.values():
            values.sort(
                key=lambda value: (
                    -value["confidence"],
                    -len(value["exact_text"]),
                    value["document_path"],
                )
            )
        theme_cards = "".join(
            f'<article><h3>{html.escape(item["name"])}</h3><b>{item["response_count"]} of {agg["response_count"]}</b><div class="bar"><i style="width:{100 * item["response_count"] / max(1, agg["response_count"]):.1f}%"></i></div><p>{html.escape(themes[item["theme_key"]]["description"])}</p></article>'
            for item in sorted(
                agg["themes"], key=lambda value: -value["response_count"]
            )
        )
        code_rows = "".join(
            f"<article><h3>{html.escape(item['name'])}</h3><b>{item['response_count']} responses</b><p>{html.escape(codes[item['code_key']]['description'])}</p>"
            + (
                "".join(
                    f'<div class="quote"><button class="copy" data-copy="{html.escape(q["exact_text"], quote=True)}">Copy</button><blockquote>{html.escape(q["exact_text"])}</blockquote><small>{html.escape(q["document_path"])}</small></div>'
                    for q in evidence[item["code_key"]][:2]
                )
                or "<small>No validated assignment.</small>"
            )
            + "</article>"
            for item in sorted(agg["codes"], key=lambda value: -value["response_count"])
        )
        codebook_rows = "".join(
            f"<article><h3>{html.escape(code['name'])}</h3><small>{html.escape(themes[code['theme_key']]['name'])}</small><p>{html.escape(code['description'])}</p><details><summary>Criteria</summary><p><b>Include:</b> {html.escape(code['inclusion_criteria'])}</p><p><b>Exclude:</b> {html.escape(code['exclusion_criteria'])}</p></details></article>"
            for code in book["codes"]
        )
        response_rows = "".join(
            f'<article><h3>{html.escape(row["document_path"])}</h3><button class="copy" data-copy="{html.escape(row["response_text"], quote=True)}">Copy response</button><p class="response">{html.escape(row["response_text"])}</p><p><b>Sentiment:</b> {html.escape(row["sentiment"].replace("_", " "))}</p><p><b>Codes:</b> {html.escape(", ".join(codes[item["code_key"]]["name"] for item in row["assignments"]) or "None")}</p></article>'
            for row in rows
        )
        gaps = [row for row in rows if row["potential_new_theme"]]
        gap_rows = "".join(
            f'<article><h3>{html.escape(row["document_path"])}</h3><p>{html.escape(row["potential_new_theme"])}</p><button class="copy" data-copy="{html.escape(row["response_text"], quote=True)}">Copy response</button><blockquote>{html.escape(row["response_text"])}</blockquote></article>'
            for row in gaps
        )
        sentiments = "".join(
            f'<div class="metric"><b>{count}</b><span>{html.escape(label.replace("_", " ").title())}</span></div>'
            for label, count in sorted(agg["sentiment_counts"].items())
        )
        payload = json.dumps(
            {"aggregate": agg, "codebook": book, "classifications": rows},
            ensure_ascii=False,
        ).replace("</", "<\\/")
        ...
        page = page.replace(
            "x.hidden=x.dataset.panel!==b.dataset.tab;);",
            "x.hidden=x.dataset.panel!==b.dataset.tab);",
        )
        target = _path(project.root, output)
        ...
        data = {
            "output": str(target),
            "response_count": len(rows),
            "theme_count": len(book["themes"]),
            "code_count": len(book["codes"]),
...
def _answer(result: Any) -> Any:
    try:
        value = result["answer"]
        return (
            value.get(QUESTION_NAME)
            if isinstance(value, dict)
            else getattr(value, QUESTION_NAME, None)
        )
...
def _parse(raw: Any) -> dict[str, Any]:
    ...
    if (
        not isinstance(sentiment.get("score"), (int, float))
        or not -1 <= sentiment["score"] <= 1
    ):
        raise ValueError("sentiment score must be between -1 and 1")
    if (
        not isinstance(sentiment.get("confidence"), (int, float))
        or not 0 <= sentiment["confidence"] <= 1
    ):
    ...
    for item in quotes:
        if not isinstance(item, dict) or not all(
            ...
            for key in ("exact_text", "rationale")
        ):
...
def jobs_command(
    ...
    try:
        ...
        if not documents:
            raise BewleyError(
                "No respondent turns were found in interview bodies.",
                code="NO_RESPONDENT_TEXT",
        ...
        if model:
            ...
            if model_target.exists() and not force:
                raise BewleyError(
                    f"{model_target} already exists", code="ALREADY_EXISTS"
                )
        ...
        data = {
            "output": str(target),
            "document_count": len(documents),
            "focused_theme_count": len(themes),
            "expected_model_calls": len(documents),
    ...
    finish(
        ...
        next_actions=[
            action(
                "run-rapid-insights",
                "Run respondent insight jobs externally",
                [
                    "ep",
                    "run",
                    str(target),
                    *model_args,
                    "--output",
                    str(target.with_name("rapid-insights.results.ep")),
                ],
                mutates_state=True,
                requires_network=True,
                requires_user_approval=True,
...
def ingest_command(
    ...
    output: Path = typer.Option(
        Path("qualitative-analysis/rapid_insights.jsonl"), "--output", "-o"
    ),
    force: bool = typer.Option(False, "--force"),
    human: bool = HumanOption,
) -> None:
    ...
    project = get_project(command, json_flag)
    result_path, jobs_path, target = (
        _path(project.root, p) for p in (results, jobs, output)
    )
    try:
        if target.exists() and not force:
            raise BewleyError(f"{target} already exists", code="ALREADY_EXISTS")
        ...
        for result in validate_result_scenarios(
            ...
            try:
                ...
                allowed = {
                    row["code_id"] for row in json.loads(scenario["themes_json"])
                }
                ...
                if any(
                    row["exact_text"] not in scenario["respondent_text"]
                    for row in insight["standout_quotes"]
                ):
...
def feedback_jobs_command(
    ...
    try:
        ...
        if not documents:
            raise BewleyError(
                "No marked AI-interviewer feedback was found.", code="NO_FEEDBACK_TEXT"
            )
        Jobs, Model, ModelList, QuestionFreeText, _, Scenario, ScenarioList = _edsl()
        scenario = Scenario(
            {
                ...
                "document_paths": {
                    row["document_id"]: row["document_path"] for row in documents
                },
                "revision_ids": {
                    row["document_id"]: row["revision_id"] for row in documents
                },
        ...
        question = QuestionFreeText(
            question_name=FEEDBACK_QUESTION_NAME,
            question_text=FEEDBACK_QUESTION_TEXT,
        )
        jobs = Jobs(survey=question.to_survey()).by(ScenarioList([scenario]))
        model_target = None
        if model:
        ...
        data = {
            "output": str(target),
            "feedback_response_count": len(documents),
            "expected_model_calls": 1,
    ...
    finish(
        ...
        next_actions=[
            action(
                "run-interviewer-feedback-insights",
                "Run one global feedback synthesis externally",
                [
                    "ep",
                    "run",
                    str(target),
                    *model_args,
                    "--output",
                    str(target.with_name("interviewer-feedback.results.ep")),
                ],
                mutates_state=True,
                requires_network=True,
                requires_user_approval=True,
...
def _parse_feedback(raw: Any) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError(
            "sentiment_distribution must be an object and standout_quotes an array"
        )
    keys = []
    for theme in themes:
        if (
        ...
        if (
            not isinstance(theme.get("response_count"), int)
            or theme["response_count"] < 0
        ):
...
def feedback_ingest_command(
    ...
    output: Path = typer.Option(
        Path("qualitative-analysis/interviewer_feedback_insights.json"),
        "--output",
        "-o",
    ),
    force: bool = typer.Option(False, "--force"),
    ...
    try:
        ...
        if len(scenarios) != 1 or len(result_rows) != 1:
            raise BewleyError(
                "Expected exactly one feedback synthesis result.",
                code="INCOMPLETE_RESULTS",
            )
        scenario = dict(scenarios[0])
        raw_answer = result_rows[0]["answer"]
        answer = (
            raw_answer.get(FEEDBACK_QUESTION_NAME)
        ...
        if sum(insight["sentiment_distribution"].values()) != len(expected):
            raise BewleyError(
                "Feedback sentiment counts must sum to the response count.",
                code="INCOMPLETE_RESULTS",
            )
        feedback = {
            row["document_id"]: row["feedback_text"]
            for row in json.loads(scenario["feedback_json"])
        }
        for theme in insight["themes"]:
            if theme["response_count"] > len(expected):
                raise BewleyError(
                    "Feedback theme count exceeds response count.",
                    code="INVALID_RESULTS",
                )
        for quote in insight["standout_quotes"]:
            if (
                quote["document_id"] not in expected
                or quote["exact_text"] not in feedback[quote["document_id"]]
            ):
                raise BewleyError(
                    "Feedback standout quote is not verbatim.", code="INVALID_RESULTS"
                )
        ...
        data = {
            "output": str(target),
            "response_count": len(expected),
            "theme_count": len(insight["themes"]),
            "ingest_log": str(log),
        }
        record_command_artifacts(project, command, locals())
    except (BewleyError, OSError, ValueError, KeyError, json.JSONDecodeError) as exc:
...
def _report_html(
    ...
    quotes = [
        ...
        for row in rows
        for quote in row["standout_quotes"]
    ]
    payload = json.dumps(
        {"rows": rows, "theme_names": theme_names}, ensure_ascii=False
    ).replace("</", "<\\/")
    theme_cards = "".join(
...
def export_command(
    input_jsonl: Path = typer.Option(
        Path("qualitative-analysis/rapid_insights.jsonl"), "--input", "-i"
    ),
    output: Path = typer.Option(
        Path("qualitative-analysis/rapid-insights.html"), "--output", "-o"
    ),
...
def feedback_export_command(
    input_json: Path = typer.Option(
        Path("qualitative-analysis/interviewer_feedback_insights.json"), "--input", "-i"
    ),
    output: Path = typer.Option(
        Path("qualitative-analysis/interviewer-feedback-insights.html"),
        "--output",
        "-o",
    ),
    ...
    try:
        ...
        theme_rows = "".join(
            ...
            for row in sorted(
                artifact["themes"], key=lambda item: -item["response_count"]
            )
        )
        sentiment_rows = "".join(
