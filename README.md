...
bewley is a local-first CLI that reads UTF-8 text corpora (plus transcribable
audio and video), stores codes, span annotations, memos, and their complete
history in an append-only event log under `.bewley/`, and writes quote tables,
interactive HTML explorers, theory diagrams, and narrative summaries.
...
**See the finished product first:** the
[interactive coded-corpus explorer](docs/adams-report.html) generated by
`bewley export html` from twenty public-domain Adams letters, and the
[HTML tutorial](docs/index.html) that builds it.

## Copy and paste into a coding agent

```text
Set up Bewley and help me complete a traceable qualitative analysis of the
text corpus in this repository.

Install `uv` if it is not already available:

python -m pip install --user --upgrade uv

Use `uv` to install Bewley in an isolated Python 3.11+ tool environment.
...
Stop if either command resolves to an unexpected environment. Let EDSL own
...
Use Bewley's CLI as the workflow source of truth:

bewley guide
bewley next

Run `bewley next` after every material stage and follow its recommendation.
Treat the JSON envelope as authoritative and check each suggested action's
mutation, network, and approval flags against my authorization.

Bewley packages open-coding work as .ep Jobs but never executes model calls;
the `ep run` step is external and needs my approval before any paid model is
selected. `bewley add-audio` and `bewley add-video` call the OpenAI
transcription API directly and also need my approval. Start with a small
pilot corpus, preserve exact source provenance, and show me candidate codes
before creating or applying them.

Preserve the event log under .bewley/, jobs.ep, models.ep, results.ep, and
the candidate-code CSV. Never silently repair, replace, or delete registered
events or results.

Continue until the analysis and exports are complete or my input or approval
is required.
```
...
- The analysis depends on traceable excerpts, memoing, code evolution, and inspectable annotation history.
...
- `.bewley/events/` and `.bewley/index/bewley.sqlite` — append-only events and rebuildable search/index state.
...
- EDSL Jobs/ModelList packages and candidate-code CSVs from `bewley open-coding`.
...
| `bewley open-coding ...` | Package EDSL jobs, ingest audited results, apply reviewed candidates. |
| `bewley version` / `guide` / `next` | Build info, lifecycle guide, and the state-aware next action. |
| `bewley codegen theory-explorer` | Generate the interactive theory-explorer renderer. |
