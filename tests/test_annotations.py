...
class TestAnnotateResolve:
    def test_resolve_conflicted_annotation(self, project: BewleyProject) -> None:
        """After a conflict, resolve should clear it with new byte ranges."""
        project.cli_ok("code", "create", "friction")
        anno_id = project.cli_ok(
            "annotate", "apply", "friction", "corpus/interview_alice.txt", "--lines", "15:17"
        ).strip()

        # Overwrite the file so the anchor can't relocate — causes conflict
        alice = project.root / "corpus" / "interview_alice.txt"
        alice.write_text("Completely different content.\n", encoding="utf-8")
        project.cli_ok("update", "corpus/interview_alice.txt")

        status = project.cli_ok("status")
        assert "conflict" in status.lower()

        # Resolve: point to bytes 0:10 in the new content
        project.cli_ok("annotate", "resolve", anno_id, "--bytes", "0:10")

        status_after = project.cli_ok("status")
        assert "conflicted_annotations\t0" in status_after or "conflict" not in status_after.lower()

    def test_resolve_nonexistent_annotation_fails(self, project: BewleyProject) -> None:
        code, _, _ = project.cli(
            "annotate", "resolve", "00000000000000000000000000000000", "--bytes", "0:10"
        )
        assert code != 0
